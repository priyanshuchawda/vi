...
This document defines practical runtime budgets for QuickCut and how we verify them after performance-related changes.
...
- Renderer initial JS payload should favor fast interactivity:
  ...
  - Lazy-load heavy secondary panels (chat, right tools panel, file panel internals).
...
- Background/idle preloading is allowed when onboarding is complete and UI is idle.
...
2. Run desktop app and record timeline in Chrome DevTools Performance (renderer process).
...
4. Regressions above budget should block merge until mitigated or accepted with rationale.
