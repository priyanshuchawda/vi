...
function runSingleCase(caseId, model) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [SINGLE_RUN_SCRIPT, '--case', caseId, '--model', model], {
      cwd: process.cwd(),
      env: process.env,
    });
    let stdout = '';
...
async function run() {
  ...
  for (const c of selectedCases) {
    ...
    const contractValid = c.expectJson ? Boolean(result.data.contractValidation?.valid) : true;
  ...
  summary.avgKeywordScore =
    summary.results.length > 0
      ? summary.results.reduce((acc, r) => acc + Number(r.keywordScore || 0), 0) /
  ...
  console.log(
    `Tokens: in=${summary.totalInputTokens} out=${summary.totalOutputTokens} total=${summary.totalTokens}`,
  );
