      ...
      >
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span
              className={`text-[9px] px-1.5 py-0.5 rounded-full border font-bold whitespace-nowrap ${statusColors[entry.status]}`}
            >
        ...
        <div className="px-3 pb-3 space-y-2 border-t border-border-primary pt-2 max-w-full overflow-hidden">
              ...
              <div className="space-y-0.5">
                {entry.visualInfo.subjects && entry.visualInfo.subjects.length > 0 && (
                  <p className="text-[11px] text-text-secondary break-words">
                    ...
                    {entry.visualInfo.subjects.join(', ')}
                  </p>
                )}
                ...
                {entry.visualInfo.dominantColors && entry.visualInfo.dominantColors.length > 0 && (
                  <p className="text-[11px] text-text-secondary break-words">
                    ...
                    {entry.visualInfo.dominantColors.join(', ')}
                  </p>
                )}
          ...
          {entry.audioInfo && (
            <div>
              ...
              <div className="space-y-0.5">
                <p className="text-[11px] text-text-secondary break-words">
                  <span className="text-text-muted">Speech:</span>{' '}
                  {entry.audioInfo.hasSpeech ? 'Yes' : 'No'}
                  {entry.audioInfo.languages &&
                    entry.audioInfo.languages.length > 0 &&
                    ` (${entry.audioInfo.languages.join(', ')})`}
                </p>
                <p className="text-[11px] text-text-secondary break-words">
                  <span className="text-text-muted">Music:</span>{' '}
                  {entry.audioInfo.hasMusic ? 'Yes' : 'No'}
                </p>
              ...
              <div className="space-y-1">
                {entry.scenes.slice(0, 5).map((scene, i) => (
                  <div key={i} className="text-[11px] flex gap-2">
                    ...
                    <span className="text-text-secondary break-words flex-1 min-w-0">
                      {scene.description}
                    </span>
                  </div>
                ))}
                {entry.scenes.length > 5 && (
                  <p className="text-[11px] text-text-muted">
                    ...and {entry.scenes.length - 5} more scenes
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="p-4 border-b border-border-primary flex-shrink-0">
        ...
        <div className="grid grid-cols-4 gap-2 text-[10px] font-bold">
          <button
            ...
            className={`px-2 py-1.5 rounded transition-all uppercase tracking-wide ${
              filterStatus === 'all'
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
        {filteredEntries.length === 0 ? (
          <div className="text-center text-text-muted text-sm mt-10">
            ...
            <p className="mb-1">
              {entries.length === 0 ? 'No media analyzed yet' : `No ${filterStatus} entries`}
            </p>
          ...
          filteredEntries.map((entry) => <MemoryEntryCard key={entry.id} entry={entry} />)
        )}
      </div>
