    ...
    <div className="flex-1 flex flex-col h-full bg-bg-secondary relative">
      ...
      <div className="p-4 border-b border-border-primary">
        <div className="flex items-center justify-between mb-1">
          ...
          {isAnalyzing && (
            <div
              className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/20"
              title={`AI is analyzing ${analyzingCount} file(s)...`}
            >
          ...
          {!isAnalyzing && memoryEntries.filter((e) => e.status === 'completed').length > 0 && (
            <div
              ...
              title={`AI has analyzed ${memoryEntries.filter((e) => e.status === 'completed').length} file(s)`}
            >
              <span className="text-[10px] font-medium text-emerald-300">
                {' '}
                {memoryEntries.filter((e) => e.status === 'completed').length}
              </span>
            </div>
          )}
        </div>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-text-secondary">
                  {(typeof clip.duration === 'number' ? clip.duration : 0).toFixed(1)}s ·{' '}
                  {clip.mediaType || 'video'}
                </div>
              </div>
            </div>
