...
import { retryAnalysis } from '../../lib/geminiMemoryService';
import type { MediaAnalysisEntry } from '../../types/geminiMemory';
...
function MemoryEntryCard({ entry }: { entry: MediaAnalysisEntry }) {
    ...
    const removeEntry = useGeminiMemoryStore((s) => s.removeEntry);
            ...
            >
                ...
                <div className="flex-1 min-w-0 overflow-hidden">
                    ...
                    {entry.status === 'analyzing' && (
                        <p className="text-[11px] text-purple-300 animate-pulse">Gemini is analyzing...</p>
                    )}
                    {entry.status === 'failed' && (
...
export default function GeminiMemoryPanel() {
    const { entries, isAnalyzing, analyzingCount, clearMemory } = useGeminiMemoryStore();
            ...
            <div className="p-4 border-b border-border-primary flex-shrink-0">
                <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-bold text-text-primary">Gemini Memory</h3>
                    </div>
                    {isAnalyzing && (
                ...
                <p className="text-xs text-text-muted mb-3">
                    Gemini analyzes your media. Memory is saved with your project.
                </p>
                ...
                {filteredEntries.length === 0 ? (
                    <div className="text-center text-text-muted text-sm mt-10">
                        ...
                        <p className="text-xs text-text-muted/60">
                            {entries.length === 0
                                ? 'Import media files and Gemini will automatically analyze them'
