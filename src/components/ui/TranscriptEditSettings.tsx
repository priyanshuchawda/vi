...
const TranscriptEditSettings = ({ isOpen, onClose }: TranscriptEditSettingsProps) => {
  const { transcriptEditSettings, updateTranscriptEditSettings, setNotification } =
    useProjectStore();
  ...
  return (
    ...
    >
      <div
        className="bg-bg-elevated border border-[#262626] rounded-xl shadow-2xl w-[90%] max-w-2xl overflow-hidden flex flex-col"
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            <div className="flex gap-3">
              ...
              <div className="text-sm text-text-primary">
                ...
                <p className="text-text-secondary">
                  These settings ensure transcript-based edits sound natural and professional,
                  avoiding audio clicks and abrupt transitions.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <label className="block text-sm font-semibold text-text-primary">
                Cut Padding:{' '}
                <span className="text-accent">{(settings.cutPadding * 1000).toFixed(0)}ms</span>
              </label>
            </div>
            <input
              type="range"
              ...
              onChange={(e) =>
                setSettings({ ...settings, cutPadding: parseInt(e.target.value) / 1000 })
              }
              className="w-full h-2 bg-bg-surface rounded-lg appearance-none cursor-pointer accent-accent"
            />
            <p className="text-xs text-text-muted mt-1">
              Extra time added before/after each cut to avoid cutting mid-phoneme. Higher values are
              safer but remove more content.
            </p>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <label className="block text-sm font-semibold text-text-primary">
                Merge Tolerance:{' '}
                <span className="text-accent">{(settings.mergeTolerance * 1000).toFixed(0)}ms</span>
              </label>
            </div>
            <input
              type="range"
              ...
              onChange={(e) =>
                setSettings({ ...settings, mergeTolerance: parseInt(e.target.value) / 1000 })
              }
              className="w-full h-2 bg-bg-surface rounded-lg appearance-none cursor-pointer accent-accent"
            />
            <p className="text-xs text-text-muted mt-1">
              Maximum gap between deleted words to merge into a single cut. Reduces the number of
              cuts for smoother flow.
            </p>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="block text-sm font-semibold text-text-primary">
                ...
                <span className="text-accent">
                  {(settings.crossfadeDuration * 1000).toFixed(0)}ms
                </span>
              </label>
            </div>
            <input
              ...
              onChange={(e) =>
                setSettings({ ...settings, crossfadeDuration: parseInt(e.target.value) / 1000 })
              }
              className="w-full h-2 bg-bg-surface rounded-lg appearance-none cursor-pointer accent-accent"
            />
            <p className="text-xs text-text-muted mt-1">
              Fade duration at cut points to eliminate clicks and pops. Longer fades are smoother
              but may sound unnatural.
            </p>
          </div>
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="text-xs text-text-muted mt-1">
                  Automatically align cuts to nearby silence regions
                </div>
              </div>
            ...
            {settings.snapToSilence && (
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-2">
                  Silence Threshold:{' '}
                  <span className="text-accent">{settings.silenceThreshold}dB</span>
                </label>
                <input
                  type="range"
                  min="-60"
                  ...
                  onChange={(e) =>
                    setSettings({ ...settings, silenceThreshold: parseInt(e.target.value) })
                  }
          ...
          <div className="border border-border-primary rounded-lg p-4 bg-bg-surface">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="text-xs text-text-muted mt-1">
                  Align cuts to video frames to avoid visual artifacts
                </div>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
            ...
            {settings.snapToFrames && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSettings({ ...settings, frameRate: parseInt(e.target.value) })
                  }
