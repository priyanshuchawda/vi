...
import type { MediaAnalysisEntry, MediaAnalysisStatus, AiMemoryContext } from '../types/aiMemory';
...
interface AiMemoryStore {
  ...
  addEntry: (
    entry: Omit<
      MediaAnalysisEntry,
      'id' | 'createdAt' | 'updatedAt' | 'status' | 'analysis' | 'tags' | 'summary'
    >,
  ) => string;
  ...
  updateAnalysis: (
    ...
    tags: string[],
    summary: string,
    extras?: Partial<MediaAnalysisEntry>,
  ) => void;
  ...
  getMemoryContext: () => AiMemoryContext;
...
export const useAiMemoryStore = create<AiMemoryStore>()((set, get) => ({
  entries: [],
  isAnalyzing: false,
  analyzingCount: 0,
  isLoaded: false,
  ...
  updateAnalysis: (id, analysis, tags, summary, extras) => {
    ...
    if (entry) {
      console.log(
        `[Memory Store]  Analysis completed for "${entry.fileName}" (will be saved with project)`,
      );
    }
  },
  ...
  getPendingEntries: () => {
    return get().entries.filter((e) => e.status === 'pending' || e.status === 'analyzing');
  },
  ...
  syncWithProject: (clipIds: string[]) => {
    ...
    if (validEntries.length !== currentEntries.length) {
      console.log(
        `[Memory Store]  Removed ${currentEntries.length - validEntries.length} orphaned entries`,
      );
