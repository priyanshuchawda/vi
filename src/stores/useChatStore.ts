...
interface ChatStore {
  ...
  exportChatForProject: () => {
    messages: ChatMessage[];
    sessionTokens: ChatStore['sessionTokens'];
  };
...
const TURN_STATUS_TRANSITIONS: Record<ChatTurnStatus, ChatTurnStatus[]> = {
  ...
  agentic_running: ['agentic_step', 'awaiting_approval', 'agentic_done', 'error', 'interrupted'],
  agentic_step: [
    'agentic_step',
    'agentic_running',
    'awaiting_approval',
...
export const useChatStore = create<ChatStore>()(
  persist(
    (set, get) => ({
      ...
      updateMessageTokens: (messageId, tokens) =>
        set((state) => {
          ...
          messages: [
            {
              ...
              role: 'system',
              content:
                'Chat cleared. How can I help with your video project? Attach files for AI analysis.',
              timestamp: Date.now(),
            },
          ],
          sessionTokens: {
