/**
 * Integration test for Gemini Memory Service
 * Tests the actual memory analysis and storage functionality
 */

import { describe, it, expect, beforeAll, afterAll, vi } from 'vitest';
import { queueMediaAnalysis, isMemoryServiceAvailable } from '../../src/lib/geminiMemoryService';
import { useGeminiMemoryStore } from '../../src/stores/useGeminiMemoryStore';
import * as fs from 'fs';
import * as path from 'path';
import { homedir } from 'os';

const TEST_VIDEO_PATH = path.join(homedir(), 'Downloads', 'videoplayback.mp4');
const MEMORY_DIR = path.join(homedir(), '.config', 'QuickCut', 'gemini_memory');
const MEMORY_FILE = path.join(MEMORY_DIR, 'memory.json');

describe('Gemini Memory Service Integration', () => {
  ...
  beforeAll(async () => {
    ...
    useGeminiMemoryStore.setState({ entries: [], isAnalyzing: false, analyzingCount: 0 });
  });
  ...
  it('should verify Gemini Memory service is available', () => {
  ...
  it('should complete analysis within reasonable time', async () => {
    ...
    while (elapsed < MAX_WAIT_MS) {
      ...
      const currentState = useGeminiMemoryStore.getState();
    ...
    const finalState = useGeminiMemoryStore.getState();
