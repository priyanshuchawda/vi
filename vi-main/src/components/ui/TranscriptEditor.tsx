...
const TranscriptEditor = () => {
  ...
  useEffect(() => {
    const activeWord = words.find(w => 
      !w.deleted && currentTime >= w.start && currentTime <= w.end
    );
    ...
    <div className="flex-1 flex flex-col bg-bg-elevated h-full">
      ...
      <div className="px-4 py-3 border-b border-border-primary">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-text-muted flex items-center gap-3">
            ...
            <span>{words.length - deletedCount} / {words.length} words</span>
          </div>
        </div>
      </div>
      ...
      {deletedCount > 0 && (
        <div className="px-4 py-2.5 bg-accent/5 border-b border-accent/20 flex items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-text-primary font-medium">~{deletedDuration.toFixed(1)}s removed</span>
          </div>
          <div className="w-px h-4 bg-border-primary"></div>
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-text-primary font-medium">{cutsCount} cut{cutsCount !== 1 ? 's' : ''}</span>
          </div>
            ...
            return (
              <span
                ...
                className={`
                  ...
                  ${isDeleted ? 'bg-red-500/20 text-red-400 line-through opacity-50 cursor-not-allowed' : 
                    isActive ? 'bg-accent/30 text-accent font-bold ring-2 ring-accent scale-105 shadow-lg shadow-accent/20' :
                    isSelected ? 'bg-blue-500/30 text-blue-300 ring-2 ring-blue-500' :
      ...
      {deletedCount > 0 && (
        <div className="px-4 py-2.5 bg-orange-500/10 border-t border-orange-500/30 text-xs text-orange-300 flex items-center gap-2">
          ...
          <span>
            <strong>{deletedCount} word{deletedCount !== 1 ? 's' : ''}</strong> marked for deletion. 
            Click <strong>"Apply to Timeline"</strong> to cut the video at these points.
          </span>
        </div>
      )}
    </div>
