...
const CHAT_MAX_TOKENS = 768;        // lowered from 1024 — output costs 4× input
const TOOL_CHAT_MAX_TOKENS = 1024;  // lowered from 1536
const HISTORY_TOOL_RESULT_MAX_TOKENS = 768; // lowered from 1024
...
async function buildMediaParts(
  ...
  for (const attachment of attachments) {
    ...
    if (attachment.mimeType.startsWith("image/")) {
      const rawBytes = attachment.base64Data
      ...
      if (rawBytes.length > INLINE_MEDIA_LIMIT_BYTES) {
        throw new Error(
          `Attachment "${attachment.name}" is ${(rawBytes.length / 1024 / 1024).toFixed(1)}MB. Nova Lite inline media limit is 25MB.`,
        );
      }
      ...
      if (msg.attachments && msg.attachments.length > 0) {
        for (const attachment of msg.attachments) {
          if (attachment.base64Data) {
            const bytes = Array.from(base64ToUint8Array(attachment.base64Data));
            const format = getMediaFormat(attachment.mimeType);
          ...
          content: [
            {
              json: {
                ...
                ...(idx === toolResults.length - 1
                  ? { _instruction: "Reply: 1) What changed, 2) Any failures, 3) Timeline diff if available, 4) Next best action." }
                  : {}),
              },
            },
          ],
